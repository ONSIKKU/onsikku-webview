...
export default function BirthGenderPage() {
  ...
  const isBirthDateAllowed = useMemo(() => {
    const parsed = parseLocalDate(localBirth);
    return !!parsed && parsed <= latestAllowedBirthDate;
  }, [latestAllowedBirthDate, localBirth]);

  const birthDateError =
    localBirth && !isBirthDateAllowed ? '만 14세 이상만 가입할 수 있어요.' : '';

  const canNext = useMemo(() => {
    return !!gender && isBirthDateAllowed && !!localNickname.trim();
  }, [gender, isBirthDateAllowed, localNickname]);
  ...
  const confirmDate = () => {
    ...
    const d = String(Math.min(tempDay, daysInMonth)).padStart(2, '0');
    setLocalBirth(`${y}-${m}-${d}`);
    setShowDatePicker(false);
  };

  const latestAllowedYear = latestAllowedBirthDate.getFullYear();
  const years = useMemo(
    () => Array.from({ length: 100 }, (_, i) => latestAllowedYear - i),
    [latestAllowedYear],
  );
  ...
  const safeTempDay = Math.min(tempDay, daysInMonth);
    ...
    <div className="flex min-h-screen flex-col bg-white pt-safe">
      ...
      {showDatePicker && (
        <div
          ...
          className="fixed inset-0 bg-black/50 flex items-end z-[60] animate-fade-in"
        >
